...
class MedecinController extends AbstractController
{
    ...
    public function new(Request $request, EntityManagerInterface $entityManager): Response
    {
        ...
        if ($form->isSubmitted() && $form->isValid()) {
            // Ajouter le contrôle pour l'email
            $email = $medecin->getEmail();
            if (!preg_match('/@gmail\.com$/', $email)) {
                $this->addFlash('error', 'L\'adresse e-mail doit se terminer par @gmail.com.');
                return $this->redirectToRoute('app_medecin_new');
            }


            $telephone = $medecin->getTel();
            if (!ctype_digit($telephone) || strlen($telephone) !== 8) {
                $this->addFlash('error', 'Le numéro de téléphone doit contenir uniquement 8 chiffres.');
                return $this->redirectToRoute('app_medecin_new');
            }
    ...
    public function edit(Request $request, Medecin $medecin, EntityManagerInterface $entityManager): Response
    {
        ...
        if ($form->isSubmitted() && $form->isValid()) {
            ...
            if (!preg_match('/@gmail\.com$/', $email)) {
                $this->addFlash('error', 'L\'adresse e-mail doit se terminer par @gmail.com.');
                return $this->redirectToRoute('app_medecin_edit', ['id' => $medecin->getId()]);
            }
